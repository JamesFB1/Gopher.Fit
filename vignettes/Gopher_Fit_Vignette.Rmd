...
tortoise$year = cut(tortoise$year, breaks = c(0,2004.5,2005.5,2007), labels = c("2004", "2005", "2006"))
...
These outliers can be explained by this table, it shows that over 2003 there was an especially high level of Shells even with an average prevalence over the sites lower than the following years.
...
```
  ...
  This next table shows total shells over all years per site, as well as the average prevalence across the years. It shows that the highest number of shells belongs to the sight with the highest average prevalence, and the lowest number of shells with the lowest average prevalence.
```{r, echo = FALSE}
