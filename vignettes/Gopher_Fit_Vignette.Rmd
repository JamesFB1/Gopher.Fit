...
Like any model within the framework of GLMM, the Poisson GLMM has 3 components to make note of. 
...
The Poisson distribution arises naturally in several ways:
* For small success probabilities and large number of trials, Poisson is a good approximation for Binomial. 
* It arises in the case when the probability of an occurrence of an event in a given time interval is proportional to the length of the interval and independent of the occurence of other events. In this case, the number of events in any unspecified interval is Poisson distributed. 
...
## Fit Model
The run_model function in this package is used to fit the tortoise data to a poisson glmm. The first parameter needed for this function is the data, and the second paramter is the name of the dataset used. 
...
se_ci_estimates <- SE_CI(tortoise)
...
This function gives an output of a list of tibbles giving the standard error, lower bound of the confidence interval and upper bound of the confidence interval. Here is the output:
...
p_value <- Hypothesis_Test(tortoise)
p_value
