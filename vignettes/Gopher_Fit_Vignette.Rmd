...
Below, we implement the Hypothesis_Test function using the tortoise data. The arguments this function requires are similar to those of the SE_CI function used earlier. However, this function requires an extra parameter called beta_i that is the name of the parameter of interest for which we wish to test the null hypothesis for. In our example, we add "prev" as the required argument for this parameter. The fitted values we pass to this function are the same as what we found above. 
